...
Chromedriver_path = 'D:/chrome_driver/chromedriver.exe'
...
def selenium_wait_xpaths(driver, tagname, time_wait=30):
    WebDriverWait(driver, time_wait).until(
        EC.presence_of_all_elements_located((By.XPATH, tagname)))
    element = driver.find_elements_by_xpath(tagname)
    return element
...
def wait_change(driver, str, time_wait=30):
    WebDriverWait(driver, time_wait).until(EC.url_contains(str))
